...
export default function AdminWasteContainersLegacyPage() {
  ...
  const stats = useMemo(() => {
    ...
    const avgFill = Math.round(
      bins.reduce((sum, bin) => sum + bin.fillLevel, 0) / total,
    );
  ...
  const selectedStatus = selectedBin ? STATUS_CONFIG[selectedBin.status] : null;
        ...
        {selectedBin && selectedStatus ? (
          <div className="citizen-v2-panel">
            <div className="flex items-center justify-between">
              ...
              <span
                className={`rounded-full px-2 py-0.5 text-[10px] font-semibold ${selectedStatus.bg} ${selectedStatus.text}`}
              >
